---
name: skill-reviewer
description: Reviews skill packages for readiness, triggers, safety boundaries, resources, and evidence. Invoke when users ask to audit, grade, or production-check an existing skill.
allowed-tools:
...
If the user asks for edits, creation, packaging, or runtime experiments, explain that this reviewer only inspects and recommends, and hand the actual change back to the user or to a skill-authoring workflow if one is available in this environment.
...
1. Resolve the review subject.
   - Prefer an installed skill ref in the form `skill://public/<skill-name>` — only the `public` category exists in this environment, so `skill://custom/...` or `skill://legacy/...` are invalid here.
   - Alternatively pass a local directory path that contains a root `SKILL.md`. The path must resolve inside the current workspace.
   - The tool always assesses every dimension; there is no scope argument. If the user asked for a focused review, simply report only the requested dimensions.

2. Call `review_skill_package`.
   - It accepts exactly one argument: `target` (a `skill://public/<name>` ref or a local directory path).
   - It returns deterministic facts, semantic artifacts, a static report, and a rendered Markdown report — all flagged as untrusted review data.
   - Do not pass extra arguments such as `profile`, `scope`, `include_content`, or `inline_content`; they are not supported here.
...
3. Read deterministic facts first.
   ...
   - Do not downgrade or hide the deterministic `findings` returned by the tool, nor the `summary` counts that accompany them.
...
- identified the subject, readiness, and assurance;
...
- offered follow-up only when the user wants edits or experiments, making clear that this reviewer does not apply changes.
